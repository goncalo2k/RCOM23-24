...
void sendFile(const char *filename)
{
    ...
    size_t filesize = lseek(filed, 0, SEEK_END);
    lseek(filed, 0, SEEK_SET);

    sendControlPacket(START, filesize);

    uint8_t buf[MAX_PAYLOAD_SIZE];
    ssize_t bytesRead = 0, packetNumber = 0;

    while ((bytesRead = read(filed, buf, MAX_PAYLOAD_SIZE)) > 0)
    {
    ...
    printf("File data sent, trying to send END packet.\n");
...
void receiveFile(const char *filename)
{
    ...
    int filed = open(filename, O_WRONLY);
    ...
    while (bytesWritten < filesize)
    {
        ...
        bytesWritten += write(filed, buf, bytesRead);
